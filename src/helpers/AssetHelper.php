...
class AssetHelper
{
     ...
     * @throws Exception
     */
    public static function fetchRemoteImage(array $urls, $fieldInfo, $api, $field = null, $element = null, $folderId = null, $newFilename = null): array
    {
        $uploadedAssets = [];

        $conflict = Hash::get($fieldInfo, 'options.conflict');

        $tempEasyApiPath = self::createTempEasyApiPath();
        ...
        foreach ($urls as $url) {
            try {
                ...
                $fetchedImage = $tempEasyApiPath . $filename;

                // But also check if we've downloaded this recently, use the copy in the temp directory
                $cachedImage = FileHelper::findFiles($tempEasyApiPath, [
                    'only' => [$filename],
                    'recursive' => false,
                ]);

                EasyApi::info('Fetching remote image `{i}` - `{j}`', ['i' => $url, 'j' => $filename]);
                ...
                $result = self::createAsset($fetchedImage, $filename, $folderId, $field, $element, $conflict, Hash::get($api, 'updateSearchIndexes'));

                if ($result) {
                    $uploadedAssets[] = $result;
                } else {
                    EasyApi::error('Failed to create asset from `{i}`', ['i' => $url]);
                }
            } catch (Throwable $e) {
                if ($field) {
                    EasyApi::error('`{handle}` - Asset error: `{url}` - `{e}`.', ['url' => $url, 'e' => $e->getMessage(), 'handle' => $field->handle]);
                } else {
                    EasyApi::error('Asset error: `{url}` - `{e}`.', ['url' => $url, 'e' => $e->getMessage()]);
                }
     ...
     * @throws Exception
     */
    public static function createBase64Image($base64, $fieldInfo, $api, $field = null, $element = null, $folderId = null): array
    {
        $uploadedAssets = [];
        ...
        foreach ($base64 as $dataString) {
            try {
                ...
                $fetchedImageWithExtension = $tempEasyApiPath . $filename;
                FileHelper::writeToFile($fetchedImageWithExtension, $decodedImage);

                $result = self::createAsset($fetchedImageWithExtension, $filename, $folderId, $field, $element, $conflict, Hash::get($api, 'updateSearchIndexes'));

                if ($result) {
                    $uploadedAssets[] = $result;
                } else {
                    EasyApi::error('Failed to create asset from `{i}`', ['i' => $dataString]);
                }
            } catch (Throwable $e) {
                EasyApi::error('Base64 error: `{url}` - `{e}`.', ['url' => $fetchedImageWithExtension, 'e' => $e->getMessage()]);
                Craft::$app->getErrorHandler()->logException($e);
                echo $e->getMessage();
            }
        }
    ...
    private static function createAsset(string $tempFilePath, string $filename, ?int $folderId, ?Assets $field, ?ElementInterface $element, string $conflict, bool $updateSearchIndexes): bool|int
    {
        ...
        EasyApi::info('Creating asset with content `{i}`', [
            'i' => Json::encode([
        ...
        if ($result) {
            ...
            if ($asset->conflictingFilename !== null && $conflict === AssetElement::SCENARIO_REPLACE) {
                ...
                if ($conflictingAsset) {
                    EasyApi::info('Replacing existing asset `#{i}` with `#{j}`', ['i' => $conflictingAsset->id, 'j' => $asset->id]);
     ...
     * @throws Exception
     */
    private static function createTempEasyApiPath(): string
    {
        $tempEasyApiPath = Craft::$app->getPath()->getTempPath() . '/easyapi/';

        if (!is_dir($tempEasyApiPath)) {
            FileHelper::createDirectory($tempEasyApiPath);
        }

        return $tempEasyApiPath;
    }
    ...
    public static function getRemoteUrlExtension($url): string
    {
        ...
        if (!$extension) {
            $client = EasyApi::$plugin->service->createGuzzleClient();
            $response = null;
