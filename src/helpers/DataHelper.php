...
class DataHelper
{
     ...
     */
    public static function fetchSimpleValue($apiData, $fieldInfo): mixed
    {
        ...
        // $value = Hash::get($apiData, $node);
        $value = $apiData[$node] ?? null;
     ...
     */
    public static function fetchArrayValue($apiData, $fieldInfo): mixed
    {
        $value = null;

        $node = Hash::get($fieldInfo, 'node');

        $dataDelimiter = EasyApi::$plugin->service->getConfig('dataDelimiter');

        // Some fields require array, or multiple values like Elements, Checkboxes, etc., and we need to parse them differently.
        // Firstly, field mapping is set up like `MatrixBlock/Images` but actual api is structured like `MatrixBlock/0/Images/0`.
        // We strip out the numbers to first find the node we've mapped to, then iterate over possible multiple values in the api.
        foreach ($apiData as $nodePath => $nodeValue) {
            // Strip out array numbers in the api path like: MatrixBlock/0/Images/0. We use this to get the field
            ...
            if ($apiPath == $node || $nodePath == $node) {
                ...
                if (is_string($nodeValue) && str_contains($nodeValue, $dataDelimiter)) {
                    ...
                    // we need the value to start as null
    ...
    public static function compareElementContent($content, $element): ?bool
    {
        ...
        foreach ($content as $key => $newValue) {
            ...
            if (is_array($existingValue) && is_array($newValue)) {
                ...
                EasyApi::debug($key . ' - diff');
                EasyApi::debug($diff);
            }

            // Now its getting personal. We need to check things per field type
            // Find the resulting value from what Easy API's field processing would produce
            // $field = Craft::$app->getFields()->getFieldByHandle($key);

            EasyApi::debug($key . ' - existing');
            EasyApi::debug($existingValue);
            EasyApi::debug($key . ' - new');
            EasyApi::debug($newValue);

            EasyApi::info('Data to update for `{i}`: `{j}`.', ['i' => $key, 'j' => Json::encode($newValue)]);
        }
