...
class DateHelper
{
    ...
    public static function parseString($value, string $formatting = 'auto'): DateTime|bool|array|Carbon|string|null
    {
        ...
        if (is_array($value)) {
            ...
            // value for an empty stringed date value in the api. At this point, we want to retain the
            // empty value in the api to overwrite the date value on the element.
        ...
        try {
            ...
            EasyApi::error('Date parse error: `{value}` - `{e}`.', ['value' => $value, 'e' => $e->getMessage()]);
    ...
    public static function parseTimeString($value): DateTime|bool|null
    {
        try {
            ...
            EasyApi::error('Time parse error: `{value}` - `{e}`.', ['value' => $value, 'e' => $e->getMessage()]);
            Craft::$app->getErrorHandler()->logException($e);
        }
