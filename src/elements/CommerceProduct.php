...
class CommerceProduct extends Element
{
    ...
    public function init(): void
    {
        ...
        // Hook into the process service on each step - we need to re-arrange the api mapping
        Event::on(Process::class, Process::EVENT_STEP_BEFORE_PARSE_CONTENT, function(ApiProcessEvent $event) {
            if ($event->api['elementType'] === ProductElement::class) {
    ...
    private function _preParseVariants($event): void
    {
        ...
        // We need to re-arrange the api-mapping from using variant-* to putting all these in a
        ...
        foreach ($api['fieldMapping'] as $fieldHandle => $fieldInfo) {
            if (str_contains($fieldHandle, 'variant-')) {
                ...
                $api['fieldMapping']['variants'][$attribute] = $fieldInfo;

                // Remove it from top-level mapping
                unset($api['fieldMapping'][$fieldHandle]);
            }
        }
    ...
    private function _checkForVariantMatches($event): void
    {
        ...
        foreach ($contentData as $handle => $value) {
            if (str_contains($handle, 'variant-')) {
                ...
                $fieldInfo = Hash::get($api, 'fieldMapping.variant-sku');
                ...
                foreach ($apiData as $nodePath => $innerValue) {
                ...
                $api['fieldUnique']['id'] = '1';
                $contentData['id'] = $variant->productId ?? 0;

                // Cleanup
                unset($api['fieldUnique'][$handle], $contentData[$handle]);
            }
        }
    ...
    private function _parseVariants($event): void
    {
        ...
        $variantMapping = Hash::get($api, 'fieldMapping.variants');
        ...
        foreach ($apiData as $nodePath => $value) {
            foreach ($variantMapping as $fieldHandle => $fieldInfo) {
                ...
                if (!in_array($apiPath, $variantFieldsByNode, true)) {
                    continue;
                }
                ...
                // Find the node in the api (stripped of indexes) that matches what's stored in field mapping
                if ($apiPath === $node) {
        ...
        foreach ($variantData as $variantContent) {
            ...
            foreach ($variantContent as $fieldHandle => $fieldInfo) {
                if (Hash::get($fieldInfo, 'attribute')) {
                    $attributeValue = DataHelper::fetchValue(Hash::get($fieldInfo, 'data'), $fieldInfo, $this->api);

                    $attributeData[$fieldHandle] = $attributeValue;
                }
            }

            // If there's no SKU in the api to process, we can't go any further, because we can very likely produce
            ...
            foreach ($variantContent as $fieldHandle => $fieldInfo) {
                if (Hash::get($fieldInfo, 'field')) {
                    ...
                    $fieldValue = EasyApi::$plugin->fields->parseField($api, $element, $data, $fieldHandle, $fieldInfo);
     ...
     */
    protected function parseAvailableForPurchase($apiData, $fieldInfo)
    {
        $value = $this->fetchSimpleValue($apiData, $fieldInfo);
     ...
     * @return bool|mixed|void
     */
    protected function parseFreeShipping($apiData, $fieldInfo)
    {
        $value = $this->fetchSimpleValue($apiData, $fieldInfo);
     ...
     * @return bool|mixed|void
     */
    protected function parsePromotable($apiData, $fieldInfo)
    {
        $value = $this->fetchSimpleValue($apiData, $fieldInfo);
     ...
     * @return false|mixed
     */
    protected function parseTaxCategoryId($apiData, $fieldInfo): mixed
    {
        $value = $this->fetchSimpleValue($apiData, $fieldInfo);
     ...
     * @return false|mixed
     */
    protected function parseShippingCategoryId($apiData, $fieldInfo): mixed
    {
        $value = $this->fetchSimpleValue($apiData, $fieldInfo);
