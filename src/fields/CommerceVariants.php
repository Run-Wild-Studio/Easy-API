...
class CommerceVariants extends Field implements FieldInterface
{
    ...
    {
        return 'easyapi/_includes/fields/commerce_variants';
    }
    ...
    public function parseField(): mixed
    {
        ...
        foreach ($value as $dataValue) {
            ...
            EasyApi::info('Search for existing variant with query `{i}`', ['i' => Json::encode($criteria)]);

            $ids = $query->ids();

            $foundElements = array_merge($foundElements, $ids);

            EasyApi::info('Found `{i}` existing variants: `{j}`', ['i' => count($foundElements), 'j' => Json::encode($foundElements)]);
        }
