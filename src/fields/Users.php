...
class Users extends Field implements FieldInterface
{
    ...
    {
        return 'easyapi/_includes/fields/users';
    }
    ...
    private function _createElement($dataValue, $groupId): ?int
    {
        ...
        if (!Craft::$app->getElements()->saveElement($element, true, true, Hash::get($this->api, 'updateSearchIndexes'))) {
            EasyApi::error('`{handle}` - User error: Could not create - `{e}`.', ['e' => Json::encode($element->getErrors()), 'handle' => $this->field->handle]);
        } else {
            EasyApi::info('`{handle}` - User `#{id}` added.', ['id' => $element->id, 'handle' => $this->field->handle]);
        }
    ...
    private function _findUsers($criteria): array
    {
        ...
        EasyApi::info('Search for existing user with query `{i}`', ['i' => json_encode($criteria)]);

        $ids = $query->ids();

        EasyApi::info('Found `{i}` existing users: `{j}`', ['i' => count($ids), 'j' => json_encode($ids)]);
