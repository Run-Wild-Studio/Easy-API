...
class Assets extends Field implements FieldInterface
{
    ...
    {
        return 'easyapi/_includes/fields/assets';
    }
    ...
    public function parseField(): mixed
    {
        ...
        foreach ($value as $key => $dataValue) {
            ...
            if ($upload && count($matches) > 0) {
                ...
                EasyApi::info('Search for existing asset with query `{i}`', ['i' => Json::encode($criteria)]);

                $ids = $query->ids();
                $foundElements = array_merge($foundElements, $ids);

                EasyApi::info('Found `{i}` existing assets: `{j}`', ['i' => count($foundElements), 'j' => Json::encode($foundElements)]);
                ...
                if ($upload && $ids && $conflict === AssetElement::SCENARIO_INDEX) {
                    ...
                    EasyApi::info('Skipping asset upload (already exists).');
                }
            }
        ...
        if ($upload) {
            if ($urlsToUpload) {
                $uploadedElements = AssetHelper::fetchRemoteImage($urlsToUpload, $this->fieldInfo, $this->api, $this->field, $this->element);
                $foundElements = array_merge($foundElements, $uploadedElements);
            }

            if ($base64ToUpload) {
                $uploadedElements = AssetHelper::createBase64Image($base64ToUpload, $this->fieldInfo, $this->api, $this->field, $this->element);
                $foundElements = array_merge($foundElements, $uploadedElements);
            }
        }
