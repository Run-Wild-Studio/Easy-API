...
class Tags extends Field implements FieldInterface
{
    ...
    {
        return 'easyapi/_includes/fields/tags';
    }
    ...
    public function parseField(): mixed
    {
        ...
        foreach ($value as $dataValue) {
            ...
            EasyApi::info('Search for existing tag with query `{i}`', ['i' => Json::encode($criteria)]);

            $ids = $query->ids();

            $foundElements = array_merge($foundElements, $ids);

            EasyApi::info('Found `{i}` existing tags: `{j}`', ['i' => count($foundElements), 'j' => Json::encode($foundElements)]);
    ...
    private function _createElement($dataValue, $groupId): ?int
    {
        ...
        if (!Craft::$app->getElements()->saveElement($element, true, true, Hash::get($this->api, 'updateSearchIndexes'))) {
            EasyApi::error('`{handle}` - Tag error: Could not create - `{e}`.', ['e' => Json::encode($element->getErrors()), 'handle' => $this->field->handle]);
        } else {
            EasyApi::info('`{handle}` - Tag `#{id}` added.', ['id' => $element->id, 'handle' => $this->field->handle]);
        }
