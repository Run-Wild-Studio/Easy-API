module.exports = {
  title: 'Easy API Documentation',
  theme: 'craftdocs',
  plugins: [['@vuepress/google-analytics', {ga: 'UA-39036834-16'}]],
  base: '/easyapi/v4/',
  shouldPrefetch: () => false,
  lang: 'en-US',
  themeConfig: {
    editLinks: true,
    docsRepo: 'runwildstudio/easyapi',
    docsDir: 'docs',
    smallerSidebarHeadings: true,
    widerSidebar: false,
    nav: [
      {
        text: 'Craft CMS',
        items: [
          {
            text: 'Craft 2 Documentation',
            link: 'https://runwildstudio.co.nz/docs/2.x/',
          },
          {
            text: 'Craft 3 Documentation',
            link: 'https://runwildstudio.co.nz/docs/3.x/',
          },
          {
            text: 'Craft 4 Documentation',
            link: 'https://runwildstudio.co.nz/docs/4.x/',
          },
          {
            text: 'Craft 2 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/api/v2/',
          },
          {
            text: 'Craft 3 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/api/v3/',
          },
          {
            text: 'Craft 4 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/api/v4/',
          },
        ],
      },
      {
        text: 'Plugins',
        items: [
          {
            text: 'Commerce 1 Documentation',
            link: 'https://runwildstudio.co.nz/docs/commerce/1.x/',
          },
          {
            text: 'Commerce 2 Documentation',
            link: 'https://runwildstudio.co.nz/docs/commerce/2.x/',
          },
          {
            text: 'Commerce 3 Documentation',
            link: 'https://runwildstudio.co.nz/docs/commerce/3.x/',
          },
          {
            text: 'Commerce 4 Documentation',
            link: 'https://runwildstudio.co.nz/docs/commerce/4.x/',
          },
          {
            text: 'Commerce 2 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/commerce/api/v2/',
          },
          {
            text: 'Commerce 3 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/commerce/api/v3/',
          },
          {
            text: 'Commerce 4 Class Reference',
            link: 'https://docs.runwildstudio.co.nz/commerce/api/v4/',
          },
          {text: 'Easy API Documentation', link: '/'},
        ],
      },
      {text: 'Craftnet API', link: 'https://docs.api.runwildstudio.co.nz/'},
    ],
    sidebar: [
      {
        title: 'Get Started',
        collapsable: false,
        children: [
          ['', 'Introduction'],
          'get-started/installation-setup',
          'get-started/requirements',
          'get-started/configuration',
        ],
      },
      {
        title: 'Feature Tour',
        collapsable: false,
        children: [
          'feature-tour/api-overview',
          'feature-tour/creating-your-api',
          'feature-tour/primary-element',
          'feature-tour/field-mapping',
          'feature-tour/importing-your-content',
          'feature-tour/trigger-import-via-cron',
          'feature-tour/using-in-your-templates',
        ],
      },
      {
        title: 'Content Mapping',
        collapsable: false,
        children: [
          'content-mapping/element-types',
          'content-mapping/field-types',
        ],
      },
      {
        title: 'Developers',
        collapsable: false,
        children: [
          'developers/field-types',
          'developers/element-types',
          'developers/data-types',
          'developers/events',
        ],
      },
      {
        title: 'Guides',
        collapsable: false,
        children: [
          'guides/importing-assets',
          'guides/importing-entries',
          'guides/importing-commerce-products',
          'guides/importing-commerce-variants',
          'guides/importing-into-matrix',
          'guides/migrating-from-expressionengine',
          'guides/migrating-from-wordpress',
        ],
      },
      {
        title: 'Troubleshooting',
        collapsable: false,
        children: ['troubleshooting'],
      },
    ],
    codeLanguages: {
      twig: 'Twig',
      php: 'PHP',
      xml: 'XML',
      json: 'JSON',
    },
  },
  markdown: {
    anchor: {
      level: [2, 3, 4],
    },
    toc: {
      format(content) {
        return content.replace(/[_`]/g, '');
      },
    },
    extendMarkdown(md) {
      require('vuepress-theme-craftdocs/markup')(md);
      md.use(require('markdown-it-deflist'));
    },
  },
};
